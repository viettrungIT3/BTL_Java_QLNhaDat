...
import quanlynhadat.Controller.HomeTransactionController;
import quanlynhadat.Models.Account;
import quanlynhadat.Models.Transaction;
import quanlynhadat.views.Login;
import static quanlynhadat.views.admin.LandTransactionsRevenueFrame.list;
import static quanlynhadat.views.admin.TransactionsRevenueFrame.list;

public class HomeTransactionsRevenueFrame extends javax.swing.JDialog {

    static List<Transaction> list = new ArrayList();
    DefaultTableModel model = new DefaultTableModel();
    int money = 0;

    public HomeTransactionsRevenueFrame(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void show1() {
        ...
        list = HomeTransactionController.getAllTransaction();
        model.setRowCount(0);
        money = 0;
        SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy");
        String type = "Giao dịch nhà";
...
//        cbYear.addItem(" ");
    ...
    private void HienThiDLTheoThang(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_HienThiDLTheoThang
        ...
        if (month.trim().compareTo("") == 0) {
            list = HomeTransactionController.getAllTransactionByYear(year);
        } else {
            list = HomeTransactionController.getAllTransactionByMonthYear(year, month);
        }
        model.setRowCount(0);
        money = 0;
        if (list.size() == 0) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            System.out.println("Name E: "+nameE);

...
public class TransactionsByUserFrame extends javax.swing.JDialog {
    ...
    private void show1() {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            model.addRow(new Object[]{i + 1, list.get(i).getT_id(), type, sdf.format(list.get(i).getT_date()), list.get(i).getMoney()});
...
//        cbYear.addItem(" ");
    ...
    private void initComponents() {
        ...
        tableHachToan.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "STT", "Mã giao dịch", "Loại giao dịch", "Ngày giao dịch", "Số tiền"
            }
        ) {
        ...
        if (tableHachToan.getColumnModel().getColumnCount() > 0) {
            ...
            tableHachToan.getColumnModel().getColumn(1).setMinWidth(120);
            tableHachToan.getColumnModel().getColumn(1).setPreferredWidth(150);
        ...
        lblHoachToan.setText("Báo cáo doanh thu cá nhân");
    ...
    private void HienThiDLTheoThang(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_HienThiDLTheoThang
        ...
        if (month.trim().compareTo("") == 0) {
            list = TransactionController.getAllTransactionByUserAndYear(idUser, year);
        } else {
            list = TransactionController.getAllTransactionByUserAndMonthYear(idUser, year, month);
        }
        model.setRowCount(0);
        money = 0;
        for (int i = 0; i < list.size(); i++) {
            ...
            model.addRow(new Object[]{i + 1, list.get(i).getId(), type, sdf.format(list.get(i).getT_date()), list.get(i).getMoney()});
            money += list.get(i).getMoney();
            txtSum.setText(String.valueOf(money));
        }
